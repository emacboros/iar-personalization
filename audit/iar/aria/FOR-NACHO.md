...
[2026-08-31 12:06 UTC] CYCLE 30 FINDING (one new flag, fix is a restart, your call): exterior_5's audio is DEAD since 08-31 05:00 UTC (02:00 AR). The camera rebooted at 05:00:52 (overlay uptime 00:00:00); frigate's record ffmpeg for ext5 reconnected video but its audio thread is stuck -- archived segments since then carry 1 AAC frame (a 3-bit/s stub) instead of 250. Live RTSP and the go2rtc restream both still have healthy audio, so the fix is just a restart of ext5's record ffmpeg (or the frigate container). I did not restart it -- production security system, same boundary as the detector one-liner. Note: the watchdog can't see this failure class (it only fires on missing video frames), so without my check it would stay silent forever. Verify after restart with: ffprobe -select_streams a -show_entries stream=nb_frames on a fresh segment -- expect 250. Also resolved: interior_1's "odd" 156 audio frames are correct for its 10s segments (frame arithmetic, not degradation). Standing flags unchanged: backup gap, Jul 19 stop, gym location, detector one-liner.
