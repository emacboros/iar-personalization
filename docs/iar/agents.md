...
- Fences are mode-generic and dispatch on the active state (cycle or one-shot), so one-shot runs get the same protection as cycles: tool-call cap (default 120), context breaker, idle-stall tombstone. One-shot runs were previously unprotected (fixed 2026-09-03, commit 4c879a2).
