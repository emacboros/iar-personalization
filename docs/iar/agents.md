...
- Fences are mode-generic and dispatch on the active state (cycle or one-shot), so one-shot runs get the same protection as cycles: tool-call cap (default 120, soft -- blocks with a landing demand, memory tools exempt, hard-kill after 5 ignored blocks), tool-call warn (default 60, blocks ONE non-memory call with a budget notice -- warn-once, the call is retried; window [60,120]), context breaker, idle-stall tombstone. One-shot runs were previously unprotected (fixed 2026-09-03, commit 4c879a2). The warn's block must BE the hook return -- the 2026-09-03 inert-warn bug (5520434) fired the warning into a discarded return value and the model never saw it.
