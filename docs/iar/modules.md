...
| c320 fix (2026-09-14, aria): `iar--cycle-terminal-echo-close` now dumps the cycle's final request before exiting (`iar--cycle-echo-close-dump`, ABORT-advice pattern: find the `gptel--request-alist` entry whose FSM `:buffer` is the cycle buffer, `process-live-p` guard, call `iar--reqlog-dump`). Without this the terminal-echo close exits the event loop before the curl sentinel/cleanup advices fire, so the final request logged START but never RESPONSE/PARSE (census: 76/76 continuo cycles 09-11..14; aria unaffected -- her cycles end via a real tool call + text response). Best-effort inside `condition-case`: a close must never fail because the witness hiccuped. Tests: `test/test-echo-close-dump.el` (dump lands, no-live no-op, dead-proc no-op). i.ar commit eff8bcb. |
