...
| `security/iar-loop-guard-chain.el` | Same-tool-chain guard (different args, unlike the identical-args guard): soft block at 10 consecutive same-tool calls, hard stop at 20. Registered via `iar--loop-guard-chain-setup` with APPEND -- the hook order is LOAD-BEARING: this guard must run AFTER `iar--loop-guard` because the bridge (`run-hook-with-args-until-success`) short-circuits at the first non-nil return; if the chain guard runs first, a blocked call never enters the identical guard's history, the chain count freezes at the soft threshold, and the hard stop is unreachable (2026-09-03 live bug: ~50 soft blocks, zero escalation, model retried through the blocks forever). Pinned by `test-chain-guard-setup-appends-not-prepends` and `test-chain-guard-escalates-through-bridge`. Provide symbol: `iar-loop-guard-chain`. |
