...
| `memory.el` | `iar-config-memory` | Memory parameters: `iar-personal-file-max-lines` (max lines injected from LOGS.md/JOURNAL.org/STATE.org, default 100). Legacy fields `iar-memory-max-entries`, `iar-memory-timeout`, `iar-memory-max-conversation-chars` remain but the summarizer module that used them is removed. |
