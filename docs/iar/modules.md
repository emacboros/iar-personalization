...
| `delegate.el` | `iar-config-delegate` | Delegate parameters: `iar-delegate-max-depth`, `iar-delegate-max-turns`, `iar-delegate-drain-grace` (fix-2 c312: seconds a timed-out delegate may keep streaming before hard abort; default 300). |
| `cycle.el` | `iar-config-cycle` | Cycle parameters: `iar-cycle-timeout`, `iar-cycle-max-turns`, `iar-cycle-tool-call-cap` (last-resort absolute tool-call bound, default 120; primary fences are the loop guards + context breaker), `iar-cycle-tool-call-warn` (early-warning at 60: blocks ONE non-memory call with a budget notice, warn-once, call is retried; warn window [60,120] -- past the cap the landing message wins). |
...
| `tool-limits.el` | `iar-config-tool-limits` | Tool limit parameters: `iar-tool-result-max-chars` (truncation), `iar-tool-result-timestamps` (wall-clock stamps), `iar-tool-result-budget` (budget trailer `[t+MM:SS/WALL cNN/CAP]`), `iar-audit-log-max-size`, `iar-request-watchdog-enabled` + `iar-request-idle-timeout` (180s) + `iar-request-total-timeout` (900s), `iar-malformed-args-feedback`, `iar-request-log-enabled` + tail/body caps. |
...
| Module | Purpose |
|--------|---------|
| `init.d/tool-call/iar-tool-call.el` | **The single integration point with gptel.** This is the ONLY module that touches gptel's internal FSM, curl internals, or tool processing. All other i.ar modules hook into this layer, not gptel directly. **Owns:** Tool registration (`iar-tool-register`, `iar-tool-make` -- wraps `gptel-make-tool` + `add-to-list`), i.ar hook variables (`iar-pre-tool-call-functions`, `iar-post-tool-call-functions`, `iar-post-response-functions` -- bridged to gptel's hooks), result truncation (`iar--truncate-tool-result` -- middle-truncation via `:around` advice on `gptel--process-tool-call`), audit logging (every tool call logged with status + args detail + agent captured at call time, in the post-tool-call bridge), token usage tracking (`iar--usage-parse-tokens` -- parses `prompt_eval_count` and `eval_count` from Ollama responses via `:before` advice on `gptel-curl--stream-cleanup` and `gptel-curl--sentinel`). Global accumulators: `iar--usage-requests`, `iar--usage-input-tokens`, `iar--usage-output-tokens`, `iar--usage-last-input`, `iar--usage-last-output`, `iar--usage-model`. `iar--usage-reset` (called at cycle start), `iar--usage-totals` (returns plist), `iar--usage-write-log` (writes to `audit/<project>/<personality>/USAGE.log` on `kill-emacs-hook`). Self-installs via `iar--tool-call-setup` at load time. **If gptel's internals change, only this file needs updating.** Requires gptel, cl-lib, subr-x, json, iar-utils, iar-audit-log. Provide symbol: `iar-tool-call`. |
| `init.d/tool-call/iar-malformed-args.el` | **Malformed-args feedback (Track A2).** Wraps tool functions (`iar-tool-make` / `iar-tool-register` -- the wrappers every tool file uses instead of `gptel-make-tool` directly) with structured error feedback for wrong argument shapes. A call with the wrong arity or type returns a `<tool_call_error>` message naming the expected spec and the received values, instead of a raw elisp error string the model cannot learn from. Owns `iar-tool-register`/`iar-tool-make` (the tool-call layer requires this module first -- a redefinition there would silently strip the wrapper). Controlled by `iar-malformed-args-feedback`. Provide symbol: `iar-malformed-args`. |
| `init.d/tool-call/iar-request-watchdog.el` | **Request watchdog (Track A1).** Aborts stalled gptel requests -- the stack has no request-level timeout, so a dead stream hangs the session silently. Tracks per-process activity via a weak-key hash (`:after` advice on `gptel-curl-get-response`, `:before` on `gptel-curl--stream-filter`), a repeating 30s timer checks stalls. Two timeout classes: idle (streaming request gone quiet mid-stream, default 180s) and total (no data at all -- covers non-streaming requests and the prompt-eval window, default 900s). Stall -> `gptel-abort` (same path as a human abort) + audit log + notice inserted into the gptel buffer so the agent sees the abort in its next request's context. Also aborts dead processes still registered in `gptel--request-alist` (the silent-hang class). Pure decision function `iar--watchdog-stall-reason` unit tested. Defcustoms in configs/tool-limits.el. Provide symbol: `iar-request-watchdog`. |
| `init.d/tool-call/iar-request-log.el` | **The witness (Track A4).** Logs every gptel request lifecycle to `audit/<project>/<agent>/REQUESTS.log` so agents can see their own emissions after the fact -- malformed tool calls, degenerate JSON, and parse failures that die before reaching the conversation buffer. Entry types: `REQ <epoch>-n START` (epoch = boot-time 12-digit yymmddHHMMSS prefix, unique across sessions -- census law: segment per-cycle by epoch, not msgs markers) (model, message count, payload tail -- the last two messages, i.e. the agent's most recent emission), `REQ <epoch>-n RESPONSE` (HTTP status + raw body tail, dumped from the process buffer before gptel destroys it), `REQ <epoch>-n PARSE` (extracted tool specs with arguments, status, error field), `REQ <epoch>-n FILTER-ERROR` (`:around` advice on `gptel-curl--stream-filter`: logs the error and the offending output chunk, then re-signals so behavior is unchanged), `REQ <epoch>-n ABORT` (partial response dumped before `gptel-abort` kills the process -- covers watchdog and human aborts). Agent name is captured at START from the conversation buffer (later events run after it may be dead). Per-fsm attribution (fix 3, c311 -- 792de9a): the agent name is ALSO stored per-process (`iar--reqlog-process-agents`, weak hash); lifecycle advices resolve the agent per-process and bind `iar--reqlog-agent-override` around their appends, so concurrent sub-agents' events land in the RIGHT agent's log (the old process-global misattributed RESPONSE/PARSE under delegation); nil override falls back to the global. Single-line sanitized entries (log-injection safe), rotated to `.1` at `iar-request-log-max-size`. Not injected into LLM context -- read deliberately via read_file for forensics. Pure helpers (`iar--reqlog-cap`, `iar--reqlog-payload-tail`, `iar--reqlog-tool-specs`) unit tested. Self-installs via `iar--reqlog-setup` at load time. Requires gptel, cl-lib, subr-x, json, iar-utils, iar-audit-log, iar-agent-utils. Provide symbol: `iar-request-log`. |
| `init.d/tool-call/iar-tool-result-timestamp.el` | **Tool result timestamping.** Prepends a wall-clock timestamp (`[HH:MM:SS]`) to every tool result -- the difference between operating blind in time and knowing 40 minutes passed between two actions. `:around` advice on `gptel--process-tool-call`, runs BEFORE truncation so the stamp survives truncated results. Idempotent (results already starting with `[` pass through). Config: `iar-tool-result-timestamps` in configs/tool-limits.el. Provide symbol: `iar-tool-result-timestamp`. |
| `init.d/tool-call/iar-tool-result-budget.el` | **Budget trailer (cycle-time fix, 2026-09-08).** Appends `[t+MM:SS/WALL cNN/CAP]` to every tool result during cycle/one-shot runs: elapsed wall-clock since cycle start / wall timeout, tool calls used (incl. current) / soft cap. STATE, not alarm -- no thresholds, no warn logic (the c67 anomaly class: threshold machinery has bugs; a constant fact cannot fail silently). OUTERMOST advice on `gptel--process-tool-call` (chain: budget -> timestamp -> truncation -> gptel); trailer at the result tail survives middle-truncation. SHARED CLOCK: reads the cycle state's `:start-time` -- the same t0 the event-loop deadline uses (one clock, two readers, no drift; pinned by test). Reads `:start-time`/`:wall-timeout` from `iar--cycle-make-state`/`iar--one-shot-make-state` and `iar-cycle-tool-call-cap` from iar-agent-cycle.el. No active state -> no trailer (interactive/delegates); state without an honest clock -> no trailer (never manufacture a t0). Config: `iar-tool-result-budget` in configs/tool-limits.el. Trailer format documented in both cycle prompts (layer 3). Provide symbol: `iar-tool-result-budget`. |

## Agent System

| Module | Purpose |
|--------|---------|
| `agent/iar-prompt-assembly.el` | **Prompt assembly engine.** Assembles a complete system prompt from three primitives: archetype + personality + project. Assembly order: base_context.org, archetype content, personality content, project objective, auto-loaded knowledge (from project `#+KNOWLEDGE`), memory injection (mode-based), mount info, container info. Key functions: `iar--assemble-prompt` (main entry, returns plist with :prompt, :tools, :mode, :archetype, :personality, :project, :containers), `iar--read-archetype`, `iar--read-personality`, `iar--read-base-context`, `iar--parse-mode` (extracts `#+MODE:` from archetype), `iar--inject-memory` (mode-based: DIGEST.md full + LOGS.md + JOURNAL.org for interactive/aria-cycle, STATE.org for autonomous/continuous, none for delegated; aria-cycle also injects the CYCLE SEQ block -- system-owned monotonic counter at audit/<project>/<personality>/CYCLE-SEQ; the block is READ-ONLY (c384: bumping inside the injection path counted suite runs and re-assemblies as cycles -- the counter counted assemblies, not cycles); the bump lives at the action site (`iar-run-cycle`, once per real cycle start, guarded on archetype aria-cycle); fixes the cycle-number duplication class c383: cycles self-numbered from generated digest/roadmap headers and the counters drifted, replaying old numbers), `iar--auto-load-knowledge` (reads doc labels from project, loads via `iar--read-knowledge-files`), `iar--filter-tools` (filters `gptel-tools` to only those in project `#+TOOLS` list; accepts optional `containers` argument -- when non-nil, `execute_code_remote` is always included in the tool list, implied by `#+CONTAINERS`), `iar--format-containers` (injects available targets + descriptions into system prompt), `iar--container-descriptions` (defconst mapping target names to brief descriptions). Requires cl-lib, subr-x, ox, iar-utils, iar-project-parser, iar-knowledge-loader, iar-mount-awareness. Provide symbol: `iar-prompt-assembly`. |
...
| `agent/iar-agent-loader.el` | **C-c a** -- Interactive agent loader (`iar-load-agent`). Prompts for personality selection, assembles prompt with interactive archetype + selected personality + default project. **C-c p** -- Personality switcher (`iar-load-personality-interactive`). Re-assembles with current archetype + project. Key state: `iar--current-archetype`, `iar--current-personality`, `iar--current-project`, `iar--current-mode`, `iar--current-containers` (all buffer-local). `iar--current-containers` is set from the assembly result's `:containers` key, making sidecar container targets available for validation by `execute_code_remote`. `iar-personality-archetype-map` (hardcoded personality-to-archetype mapping, includes `("pentest" . "interactive")`). `iar--archetype-for-personality` (lookup), `iar-personality-project-map` (explicit personality->project overrides, e.g., bessie -> moto), `iar--project-for-personality` (map override first, then project file matching personality name, else "iar"). `iar--setup-assembled-buffer` (calls `iar--assemble-prompt`, sets all buffer-local state, resets knowledge state). Backward compat: `iar--current-agent-name` = personality name, `iar--current-agent-file` = personality file path. Requires cl-lib, subr-x, iar-agent-utils, iar-utils, iar-prompt-assembly, iar-project-parser. Provide symbol: `iar-agent-loader`. |
...
| `agent/iar-agent-cycle.el` | Autonomous agent cycle runner (`iar-run-cycle`). Any personality with an autonomous/continuous archetype can run autonomously in a loop. Has its own cycle buffer (`*<agent>-cycle*`), timeout, max turns, continue prompting. Supports `:agent`, `:knowledge`, `:self-modification`, `:timeout`, `:prompt` parameters. `iar-personality-cycle-map` maps personalities to cycle files (darwin -> self_modification, gardener -> monitoring, librarian -> documentation_sync). Per-agent cycle prompt loading (`<personality>_cycle.org` fallback to `agent_cycle.org`). Continue prompt loading (`agent_cycle_continue.org` fallback to defconst). Completion detection via `iar--cycle-complete-p` (LOOP_COMPLETE vs CYCLE_COMPLETE markers). Continuation via `gptel-post-response-functions` hook. Cycle logging to `audit/<project>/<personality>/cycle.log`. Token usage tracking. Telegram notification on cycle end via `kill-emacs-hook`. Batch mode event loop: `accept-process-output` with FSM state monitoring. Uses `iar--archetype-for-personality` and `iar--project-for-personality` to resolve archetype and project from personality name. Exit codes: 0 (cycle complete), 1 (timeout), 2 (LOOP_COMPLETE). **Dead-cycle guard (c326):** a failed request (start==end post-response, e.g. HTTP 429 quota) with no live successor -- no non-terminal FSM in `gptel--request-alist` (`iar--request-successor-live-p` scans the alist; entries are `(key fsm . cleanup)`, fsm = cadr) and no process on the run buffer -- ends the cycle IMMEDIATELY (exit 1) instead of idling the full 1800s stall window: gptel re-sends only after a tool result, so a failed request can never reach strike 3 (the 2026-09-13 quota storm: 16 cycles x 30min = 8h of sophon wall-clock on dead requests). Applies to both the cycle and one-shot event loops. `iar-darwin-run-cycle` backward compat alias. **Abort-aware re-prompt (c80, 9ceb390):** a guard-aborted turn whose partial thinking text landed in the buffer takes the SUCCESS path (START < END), so the dead-cycle guard's strike counting never sees it -- live-fire 09-18: 22 guard-aborted cycle turns re-prompted with the STANDARD continue prompt (law 41 violated at cycle level; one boot burned 9 aborts in 4 min re-entering the same thinking pattern). Witness is `iar--reqlog-last-abort` (set by the reqlog's `gptel-abort` advice, consumed+cleared on read; inference from absent stop/token data rejected -- nil is also the reqlog-disabled shape). Aborted turns get the SHARED abort-continue prompt (`iar--abort-continue-prompt`, moved to `shared/iar-agent-utils.el` because delegate loads before agent-cycle) and count consecutive strikes against `iar-cycle-abort-reprompts` (default 2); past the cap the cycle ends LOUD (exit 1). **One-shot mode:** `iar-run-one-shot` function for single-instruction execution. Forces archetype to "one-shot" (not from personality map). Reads prompt from `IAR_ONE_SHOT_PROMPT` env var. Completion detected via `iar--one-shot-extract-response` (searches for `iar-one-shot-response-open`/`iar-one-shot-response-close` delimiters). Nudge prompt sent when agent produces text without delimiters. Final response printed to stdout via `princ`. Separate state: `iar--one-shot-state`. Fences (tool-call cap, context breaker, idle tombstone) dispatch on active state and cover one-shot; timeout without a final response exits 1; the one-shot tracker is global, not buffer-local. Requires gptel, cl-lib, subr-x, json. Provide symbol: `iar-agent-cycle`.  Cycle state carries `:start-time`/`:wall-timeout` (captured once at make-state) -- the shared clock: both event loops compute their deadline from it and the budget trailer reads it (one t0, two readers, no drift). |
...
| Module | Tool | Purpose |
|--------|------|---------|
| `tools/agent/delegate.el` | `delegate` | Async multi-agent delegation. Agent parameter is optional -- when omitted, defaults to agent-assistant (pipeline mode). When specified, spawns that personality directly (direct mode). Spawns sub-agent buffers with timeout handling, max depth limiting, unknown tool blocking, text-only turn re-prompting. Resolves archetype and project from personality name via `iar--archetype-for-personality` and `iar--project-for-personality`. Assembles prompt via `iar--assemble-prompt`. Applies tool gating from project `#+TOOLS` (depth strip runs AFTER the project set -- c308 fix 763c7b3). Result extraction via `=== DELEGATION RESULT ===` marker. Timeout path (fix-2 c312, 2f7c82c): `iar--delegate-live-subrequests-p` checks for a live curl request against the delegate buffer; if the pipeline is still streaming the handler DRAINS (1s re-checks, completed-sym stays nil so the completion hook delivers the real result) up to `iar-delegate-drain-grace` seconds, then falls through to the c149 abort path (`iar--delegate-timeout-abort`, extracted). If nothing is live, the c149 abort path applies directly. Pipeline: agent-assistant (depth 1) delegates to implementer/reviewer (depth 2), correction loop limited by archetype prompt (default 2 rounds). Guard-abort turns (c71, extended c79, 3b6dc8f): START == END (thinking-loop-guard abort shape) is checked FIRST, before the tools-called check -- an aborted turn is a FAILED turn regardless of prior tool use (the c79 hole: an abort on a turn after tool use hit the tools-called branch and the raw buffer shipped as a 'completed' result). Aborted turns get the abort-aware re-prompt and count strikes against `iar-delegate-abort-reprompts` (default 2); past the cap the delegate ends LOUD. Provide symbol: `iar-delegate-tool`. |
...
| `tools/agent/reload_agent.el` | `reload_agent` | Re-read personality .org and update system prompt in current buffer. Re-assembles via `iar--setup-assembled-buffer`. With an explicit agent name: re-resolves archetype + project from the personality (same resolution as delegate / C-c a). Without: refreshes current personality with current archetype + project. Provide symbol: `iar-reload-agent`. |
...
| `security/iar-audit-log.el` | Audit logging for tool calls. Log at `audit/audit.log`. `iar--audit-log` (tool detail) is the base writer -- best-effort via condition-case, nil agent logged as 'unknown'. `iar--audit-log-as` (agent tool detail) writes with a pre-captured agent name (used by the tool-call bridge, which captures the agent in the conversation buffer's dynamic context before async sentinels lose it). `iar--audit-log-tool-call-with-agent` (tool-name args result agent) builds the per-tool detail: write_file/append_file -> target path, execute_code_local/remote -> command capped 200 chars, git_commit -> repo + message capped 80, other tools -> name/status/result_len only. `iar--audit-log-agent-name` captures the agent name, never signals. Legacy wrappers `iar--audit-log-write`/`append`/`exec` retained (write_roadmap uses -write). `iar--audit-redact-secrets` (2026-09-18, relay 0081 structural fix): redacts secret-shaped strings (classic PAT `ghp_`+36, fine-grained `github_pat_`+40, AWS `AKIA`+16) inside `iar--audit-sanitize-detail`, so every audit + REQUESTS.log entry passing the sanitizer redacts live credentials -- the log keeps the evidence a secret EXISTED without keeping the secret. Pasted by a human in session 09-17, captured by REQUESTS.log, propagated to git history (187c4fa6); revocation is the human's, this is the structural backstop. Tested in test-audit-detail.el (7 redact tests + integration). Rotation at `iar-audit-log-max-size` (from configs/debug.el). Requires subr-x, iar-utils. Provide symbol: `iar-audit-log`. |
...
| `security/iar-loop-guard-chain.el` | Same-tool-chain guard (different args, unlike the identical-args guard): soft block at 10 consecutive same-tool calls, hard stop at 20. CONVERGENCE RESET (2026-09-03, commit b1eb7e0): the counter counts the TOOL, but converging investigation (ssh -> curl -> grep) is a different behavior that shares the tool -- five production witness sets of legitimate investigation blocked at 10. The guard keeps its OWN history ring (`iar--chain-history`, entries `(NAME MD5 ARGS)`; the identical guard's ring stores only the args md5, and similarity cannot be measured on a hash). When a same-tool call's args score below `iar-loop-guard-chain-similarity` (default 0.5, token-set Jaccard, one-char tokens dropped) against the previous same-tool call's args, the chain counter resets: iterators keep their monotone count (tail -N = 1.0, git-log paging = 0.67), investigators get their counter wiped (ssh-vs-curl = 0.07). Trailing identical calls are skipped before counting (identical guard's domain). Registered via `iar--loop-guard-chain-setup` with APPEND -- the hook order is LOAD-BEARING: this guard must run AFTER `iar--loop-guard` because the bridge (`run-hook-with-args-until-success`) short-circuits at the first non-nil return; if the chain guard ran first, its blocks would prevent the identical guard from ever seeing those calls and its escalation would be unreachable (2026-09-03 live bug: ~50 soft blocks, zero escalation, model retried through the blocks forever). Pinned by `test-chain-guard-setup-appends-not-prepends`, `test-chain-guard-escalates-through-bridge`, and the convergence-reset tests (investigation walk must NOT block; iterator and git-log paging MUST still block). Provide symbol: `iar-loop-guard-chain`. |
...
- Interactive: LOGS.md (last N lines) + JOURNAL.org (last N lines) from `audit/<project>/<personality>/`
...
| Post-close request suppression (empty-end fix, c348): the terminal-echo close sets :completed and blocks the echo call, but gptel's tool loop still transitions TRET -> WAIT and fires ONE MORE request; the model answers empty (0/0), the tombstone kills a finished cycle (14 empty-ends in continuo's log, every one immediately after the echo request). Fix: `iar--cycle-suppress-post-close-wait` (:around advice on `gptel--handle-wait`, registered globally at module load) -- when the active run is :completed, the WAIT handler does not fire; no request is sent, nothing left in flight. Fail-open on gate error. Tests: `test/test-post-close-suppress.el` (gate blocks after completed, passes before completed / no state / one-shot, advice registered). Suite 1280/1280. |
| Request-log orphan fix (terminal-echo exit path): v1 (c320, eff8bcb) dumped the live request at the terminal-echo close -- LIVE VERIFICATION FAILED (c322): the echo request completes normally (its PARSE even shows the close firing); the orphan is the POST-CLOSE RE-SEND (gptel's tool loop re-sends after every tool result, including a blocked one) which the event loop kill-emacs's mid-flight. v2 (c323, 22d64ae + 5557d89): `iar--cycle-exit-dump`, called from the exit paths of `iar-run-cycle` AND `iar-run-one-shot` before `kill-emacs` -- dumps EVERY live request in `gptel--request-alist` (post-close re-send, delegate requests, summary requests racing the grace window; every exit path with a request in flight). The `gethash iar--reqlog-processes` guard prevents double-dumping completed requests. Tests: `test/test-echo-close-dump.el` (v1) + `test/test-exit-dump.el` (v2, law-39 production-sequence replay: echo completes -> re-send in flight -> exit dumps it). Suite 1269/1269. |
