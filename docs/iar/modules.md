...
| `init.d/tool-call/iar-request-log.el` | **The witness (Track A4).** Logs every gptel request lifecycle to `audit/<project>/<agent>/REQUESTS.log` so agents can see their own emissions after the fact -- malformed tool calls, degenerate JSON, and parse failures that die before reaching the conversation buffer. Entry types: `REQ n START` (model, message count, payload tail -- the last two messages, i.e. the agent's most recent emission), `REQ n RESPONSE` (HTTP status + raw body tail, dumped from the process buffer before gptel destroys it), `REQ n PARSE` (extracted tool specs with arguments, status, error field), `REQ n FILTER-ERROR` (`:around` advice on `gptel-curl--stream-filter`: logs the error and the offending output chunk, then re-signals so behavior is unchanged), `REQ n ABORT` (partial response dumped before `gptel-abort` kills the process -- covers watchdog and human aborts). Agent name is captured at START from the conversation buffer (later events run after it may be dead). Single-line sanitized entries (log-injection safe), rotated to `.1` at `iar-request-log-max-size`. Not injected into LLM context -- read deliberately via read_file for forensics. Pure helpers (`iar--reqlog-cap`, `iar--reqlog-payload-tail`, `iar--reqlog-tool-specs`) unit tested. Self-installs via `iar--reqlog-setup` at load time. Requires gptel, cl-lib, subr-x, json, iar-utils, iar-audit-log, iar-agent-utils. Provide symbol: `iar-request-log`. |
