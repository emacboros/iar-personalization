...
for CAM in $cams; do
  ...
  RESP=$(curl -s --max-time 300 "$OLLAMA" -d "{\"model\":\"qwen3.6:35b-a3b\",\"prompt\":\"This is a security camera frame. Answer with ONLY the camera name it most likely shows, choosing from: front-door, backyard, driveway, street, balcony, living-room, bedroom, kitchen, garage, garden, other. One word or hyphenated pair, nothing else.\",\"images\":[\"$B64\"],\"stream\":false,\"keep_alive\":-1}" | jq -r '.response' 2>/dev/null | tr -d '[:space:]' | head -c 40)
